...
class CreateTripViewController: UIViewController {
        ...
        if let name = tripTitleBox.text {
            ...
            if let trip = trip {
                do {
                    ...
                    createFailAlert(message: "Failed to save the trip", error: error as! String, parent: self)
                }
            } else {
                createFailAlert(message: "Failed to create the trip", error: "Error", parent: self)
            }
        }
    }
    ...
    func checkSave() -> Bool {
        if tripDescriptionTextView.text == "" {
            self.presentAlert(title: "Error", message: "You must have a description before saving the trip!")
            return false
        } else if tripTitleBox.text == "" {
            self.presentAlert(title: "Error", message: "You must have a title before saving the trip!")
            return false
        }
